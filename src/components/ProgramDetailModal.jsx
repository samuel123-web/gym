  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-content" style={{ maxWidth: '750px' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="program-image-wrapper" style={{ height: '260px', borderRadius: 'var(--radius-md)', marginBottom: '1.5rem' }}>
          <img src={program.image} alt={name} className="program-image" />
          <span className="program-category-tag">{category}</span>
        </div>

        <h2 style={{ fontSize: '2.25rem', marginBottom: '0.5rem' }}>{name}</h2>
        <p style={{ fontSize: '1rem', color: 'var(--text-muted)', marginBottom: '1.5rem', lineHeight: '1.6' }}>
          {fullDescription}
        </p>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.5rem', backgroundColor: 'var(--bg-input)', padding: '1rem', borderRadius: 'var(--radius-md)' }}>
          <div>
